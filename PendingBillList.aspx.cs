...
{
    public partial class PendingBillList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (Session["UserType"] != null && Session["UserType"].ToString() == "Accountant")
                {
                    ...
                    gvProjectList.Columns[3].Visible = false;
                }
            }
        }
        ...
        private void LoadProjectList()
        {
            ...
            string query = "SELECT * FROM TablePendingBill";
        ...
        protected void gvProjectList_RowCommand(object sender, System.Web.UI.WebControls.GridViewCommandEventArgs e)
        {
            ...
            try
            {
                if (e.CommandName == "EditRow")
                {
                    using (SqlConnection con = new SqlConnection(connStr))
                    {
                        SqlCommand cmd = new SqlCommand("SELECT * FROM TablePendingBill WHERE Id=@Id", con);
                        ...
                        if (dr.Read())
                        {
                            ...
                            txtProjectName.Text = dr["Title"].ToString();
                            txtProjectArea.Text = dr["Remarks"].ToString();
                    ...
                    using (SqlConnection con = new SqlConnection(connStr))
                    {
                        ...
                        if (dr.Read())
                        {
                            ...
                            <table style='width:100%; border-collapse: collapse; text-align:left; margin-top:10px;'>
                                <tr>
                                    <td style=' padding: 4px 8px; width: 150px;border:1px solid white;'>টাইটেল:</td>
                                    <td style='padding: 4px 8px;border:1px solid white;'>{dr["Title"]}</td>
                                </tr>
                                <tr>
                                    <td style='padding: 4px 8px; border:1px solid white;'>বিবরণ:</td>
        ...
        protected void btnSaveEdit_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(connStr))
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("UPDATE TablePendingBill SET Title=@ProjectName, Remarks=@Area, Status=@Status WHERE Id=@Id", con);
        ...
        protected void btnYesDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(connStr))
                {
                    SqlCommand cmd = new SqlCommand("DELETE FROM TablePendingBill WHERE Id=@Id", con);
        ...
        protected void btnSaveAdd_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(connStr))
            {
                SqlCommand cmd = new SqlCommand("INSERT INTO TablePendingBill (Title, Remarks, Status) VALUES (@ProjectName, @Area, @Status)", con);
