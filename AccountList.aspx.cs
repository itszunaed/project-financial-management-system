...
namespace TalukderEngineering
{
    public partial class AccountList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (Session["UserType"] != null && Session["UserType"].ToString() == "Accountant")
                {
                    ...
                    gvProjectList.Columns[2].Visible = false;
                }
            }
        }
        ...
        private void LoadProjectList()
        {
            ...
            string query = "SELECT * FROM TableAccountList";
        ...
        protected void gvProjectList_RowCommand(object sender, System.Web.UI.WebControls.GridViewCommandEventArgs e)
        {
            ...
            try
            {
                if (e.CommandName == "EditRow")
                {
                    using (SqlConnection con = new SqlConnection(connStr))
                    {
                        SqlCommand cmd = new SqlCommand("SELECT * FROM TableAccountList WHERE Id=@Id", con);
                        ...
                        if (dr.Read())
                        {
                            ...
                            txtProjectName.Text = dr["AccountName"].ToString();
                            

                            ddlProjectStatus.SelectedValue = dr["AccountantAccess"].ToString();
                    ...
                    using (SqlConnection con = new SqlConnection(connStr))
                    {
                        ...
                        if (dr.Read())
                        {
                            ...
                            <table style='width:100%; border-collapse: collapse; text-align:left; margin-top:10px;'>
                                <tr>
                                    <td style=' padding: 4px 8px; width: 150px;border:1px solid white;'>এ্যাকাউন্টের নাম:</td>
                                    <td style='padding: 4px 8px;border:1px solid white;'>{dr["AccountName"]}</td>
                                </tr>
                                
        
                                <tr>
                                    <td style='padding: 4px 8px; border:1px solid white;'>এ্যাকাউন্টেন্ট এক্সেস:</td>
                                    <td style='padding: 4px 8px; border:1px solid white;'>{dr["AccountantAccess"]}</td>
                                </tr>
        ...
        protected void btnSaveEdit_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(connStr))
            {
                try
                {
                    ...
                    // Get the old name
                    SqlCommand getOldNameCmd = new SqlCommand("SELECT AccountName FROM TableAccountList WHERE Id = @Id", con);
                    getOldNameCmd.Parameters.AddWithValue("@Id", rowId);
                    oldProjectName = getOldNameCmd.ExecuteScalar()?.ToString();

                    SqlCommand cmd = new SqlCommand("UPDATE TableAccountList SET AccountName=@ProjectName,  AccountantAccess=@Status WHERE Id=@Id", con);
                    ...
                    SqlCommand updateProjectsCmd = new SqlCommand("UPDATE TableProjectDetails SET AccountName = @NewName WHERE AccountName = @OldName", con);
        ...
        protected void btnYesDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(connStr))
                {
                    SqlCommand cmd = new SqlCommand("DELETE FROM TableAccountList WHERE Id=@Id", con);
        ...
        protected void btnSaveAdd_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(connStr))
            {
                SqlCommand cmd = new SqlCommand("INSERT INTO TableAccountList (AccountName, AccountantAccess) VALUES (@ProjectName,  @Status)", con);
